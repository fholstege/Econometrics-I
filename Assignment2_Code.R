...
simulationResults<- function(nSimulations, nSample, a, b1,b2,c1,c2, a0,b0,sigma2){
  ...
  for(i in 1:nSimulations){
    ...
    # define the errors, save them
    sigma2_i = sigma2 * exp((c1* vZ^2)+ (c2 * vZ))
    errors = rnorm(nSample,0,sd = sigma2_i)
    
    # save param for plots
    param <- rbind(param, data.frame(X1 = mX1, X2 = mX2, Errors = errors))
...
resultsMelted <- melt(dfResultSim$dfResults, id.vars = "Estimator") 
...
# make plot for the errors
dfParam <- dfResultSim$dfParam
par(mfrow = c(2,1))
plot(dfParam$X1, dfParam$Errors, main = "Errors and X1", xlab = "X1", ylab = "Error", ylim = c(-10,10))
plot(dfParam$X2, dfParam$Errors, main = "Errors and X2", xlab = "X2", ylab = "Error", ylim = c(-10,10))
